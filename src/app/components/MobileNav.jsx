...
const MobileNav = () => {
  ...
  return (
    <nav className=" absolute top-[150%] w-full md:hidden font-medium flex flex-col justify-between rounded-lg border border-black">
      <div className=" flex justify-between items-center">
        ...
        <span className=" flex justify-center items-center border-l border-black p-2" onClick={() => setMenuClose(!menuClose)}>
          <Icon
            icon="ic:round-menu"
            width="35"
            height="35"
      ...
      {menuClose && <ul className=" w-full bg-[#FFF5F0] rounded-b-lg">
        {navLinks.map((navlink) => (
          <li key={navlink.title}>
            <Link
              href={navlink.link}
              onClick={() => setMenuClose(true)}
              className={`${navlink.title == "Contact" ? "rounded-b-lg" : ""} ${
